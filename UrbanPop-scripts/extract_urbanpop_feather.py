...
def print_header(df):
    ...
    #hash_define_hdr_fname = ''.join(['_' + i if i.isupper() else i.upper() for i in hdr_fname.replace('.', '')]).lstrip('_') + '_'
    #"#ifndef " + hash_define_hdr_fname + "\n" \
    #"#define " + hash_define_hdr_fname + "\n\n" \
    hdr = "/*! @file " + hdr_fname + "\n" \
    ...
    for field_type in df:
        if field_type in categ_types:
            ...
            hdr += 'static string ' + field_type + '_descriptions[] = {"' + '", "'.join(categs_expected) + '"};\n'
    hdr += '\n'
